...
import EditMedicionPrecioForm from './EditMedicionPrecioForm';
...
export default function MedicionesListSection() {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (!confirm('¿Estás seguro de que quieres eliminar esta hoja de mediciones precio?')) {
      return;
    }
    try {
      ...
      alert('Error al eliminar la hoja de mediciones precio. Por favor, intenta de nuevo.');
    }
  };
  ...
  const handleUpdate = async (updatedData: any) => {
    ...
    try {
      ...
      alert('Error al actualizar la hoja de mediciones precio. Por favor, intenta de nuevo.');
    }
  };
  ...
  const handleExportExcel = async (medicion: Medicion) => {
    try {
      const { exportToExcel } = await import('utils/exportMedicionPrecio');
  ...
  const handleExportPDF = async (medicion: Medicion) => {
    try {
      const { exportToPDF } = await import('utils/exportMedicionPrecio');
  ...
  if (loading) {
    return (
      <Wrapper>
        <SectionTitle>Lista de Hojas de Mediciones Precio</SectionTitle>
  ...
  if (mediciones.length === 0) {
    return (
      <Wrapper>
        <SectionTitle>Lista de Hojas de Mediciones Precio</SectionTitle>
        <EmptyState>No hay hojas de mediciones precio registradas.</EmptyState>
      </Wrapper>
    );
  }
  ...
  return (
    <Wrapper>
      ...
      <EditModal
        ...
        title="Editar Hoja de Mediciones Precio"
      >
        {editingMedicion && (
          <EditMedicionPrecioForm
      ...
      <MedicionesGrid>
        {mediciones.map((medicion) => (
          <MedicionCard key={medicion.id}>
            <CardHeader>
              <HeaderTitle>HOJA DE MEDICIONES PRECIO</HeaderTitle>
              <HeaderContent>
                <HeaderLeft>
                  <ProjectInfo>
                    <ProjectRow>
